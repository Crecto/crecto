...
describe "Crecto::Adapters::SQLite3" do
  ...
  it "should generate insert query" do
    u = SQLiteRepo.insert(User.from_json(%({ "name": "chuck", "yep": false })))
    check_sql do |sql|
      sql.should eq([
  ...
  it "should generate get query" do
    user = SQLiteRepo.insert(User.from_json("{ \"name\":\"lucy\" }"))
    Crecto::Adapters.clear_sql
    SQLiteRepo.get(User, user.instance.id)
    check_sql do |sql|
  ...
  it "should generate update queries" do
    changeset = SQLiteRepo.insert(User.from_json(%({ "name": "linus", "yep": true })))
    Crecto::Adapters.clear_sql
    changeset.instance.name = "snoopy"
    changeset.instance.yep = false
    SQLiteRepo.update(changeset.instance)
    check_sql do |sql|
      sql.should eq([
  ...
  it "should generate delete queries" do
    changeset = SQLiteRepo.insert(User.from_json(%({ "name": "sally" })))
    Crecto::Adapters.clear_sql
    SQLiteRepo.delete(changeset.instance)
    check_sql do |sql|
