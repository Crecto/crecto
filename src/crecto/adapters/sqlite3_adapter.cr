  ...
  module Adapters
    ...
    module SQLite3
      ...
      private def self.insert(conn, changeset, tx : DB::Transaction?)
        ...
        res = exec_execute(conn, q.join(" "), fields_values[:values], tx)
        execute(conn, "SELECT * FROM #{changeset.instance.class.table_name} WHERE #{changeset.instance.class.primary_key_field} = #{res.last_insert_id}")
      end
